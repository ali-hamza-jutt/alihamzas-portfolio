import Work1 from "../../assets/work1.png";
import Work2 from "../../assets/work2.png";
import Work7 from "../../assets/work7.gif";
import Work8 from '../../assets/work8.png';
import Work9 from '../../assets/work9.png';
import Work10 from '../../assets/work10-2.png';
import Work11 from '../../assets/work11.png';
import Work12 from '../../assets/work12.png';
import Work13 from '../../assets/work13.jpeg';


export const projectsData = [
  {
    id: 0,
    gif: Work7,
    title: "Dive Deep into My Code Base",
    category: "all",
    link: "https://github.com/ali-hamza-jutt",
  },
    {
    id: 13,
    gif: Work13,
    title: "Smart marketplace with messaging, reels, wallet, and live tracking",
    category: "all",
    link: "https://ojiyo.ai",
  },
  {
    id: 12,
    image: Work12,
    title:
      "Slay Cash is an advanced play-to-earn platform developed with Next.js, Tailwind CSS, and SQL. It provides a secure authentication system, seamless social media integration for earning rewards, a robust payment gateway, and comprehensive profile management. Players can enjoy engaging gameplay while monetizing their skills and sharing their achievements on social media.",
    category: "all",
    link: "https://slay-cash.vercel.app",
  },
  // {
  //   id: 11,
  //   image: Work11,
  //   title:
  //     "Digitize Web Landing Page is developed with React, featuring scalable components and smooth animations using Framer Motion, ensuring a fast and engaging user experience.",
  //   category: "frontend",
  //   link: "https://digitilize-web-clone.vercel.app/",
  // },
  // {
  //   id: 10,
  //   image: Work10,
  //   title:
  //     "Craft a stunning, responsive landing page using React.js, showcasing products with diverse card styles and cutting-edge design.",
  //   category: "frontend",
  //   link: "https://ali-hamza-perfume-parlor-landing.vercel.app/",
  // },
];

export const projectNav = [
  { name: "all" },
  { name: "full stack" },
  { name: "frontend" },
  // { name: "design" },
];
